apply_model = function(mp, mri_data, mri_fu_data, mri_wm_data=NULL, mri_wm_fu_data=NULL, 
                       covX_test, signif_indx, lasso_covB, lasso_covB_fu = NULL, mask, 
  ...
  if (!is.null(n_voxels_mask) && any(lasso$i <= n_voxels_mask)) { # if the model contains at least one voxel
    if (!is.null(mri_data) ){
      ...
      if(!is.null(covX_site)) {
        mri_test = combat_apply(combat$gm, mri_test, covX_site, mod=covX_test, verbose = FALSE)$dat.combat # ALEIX, AIXÒ ÉS SI NO PRECALCULAT. PER PRECALCULAT, NO S'HA DE POSAR "mod"
      }
      # FI combat
      img_3d = mri_data[,,,1]
      img_3d = img_3d * 0
    ...
    if (!is.null(mri_fu_data)) {
      ...
      mri_test = matrix(unlist(mri_test), nrow = n_subj, byrow = TRUE)
      if(!is.null(covX_site)) {
        mri_test = combat_apply(combat$gm_mod, mri_test, covX_site, mod=covX_test, verbose = FALSE)$dat.combat # ALEIX, AIXÒ ÉS SI NO PRECALCULAT. PER PRECALCULAT, NO S'HA DE POSAR "mod"
      }
    ...
    if (!is.null(mri_wm_data)) {
      ...
      mri_test = matrix(unlist(mri_test), nrow = n_subj, byrow = TRUE)
      if(!is.null(covX_site)) {
        #load(sprintf("combat_un_wm%s.Rdata",name_combat))
        mri_test = combat_apply(combat$wm, mri_test, covX_site, mod=covX_test, verbose = FALSE)$dat.combat # ALEIX, AIXÒ ÉS SI NO PRECALCULAT. PER PRECALCULAT, NO S'HA DE POSAR "mod"
      }
    ...
    if (!is.null(mri_wm_fu_data)) {
      ...
      mri_test = matrix(unlist(mri_test), nrow = n_subj, byrow = TRUE)
      if(!is.null(covX_site)) {
        #load(sprintf("combat_fu_wm%s.Rdata",name_combat))
        mri_test = combat_apply(combat$wm_mod, mri_test, covX_site,mod=covX_test, verbose = FALSE)$dat.combat # ALEIX, AIXÒ ÉS SI NO PRECALCULAT. PER PRECALCULAT, NO S'HA DE POSAR "mod"
      }
